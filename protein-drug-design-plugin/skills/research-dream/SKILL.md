---
name: research-dream
description: Research Dream。面向科研场景的记忆巩固技能。在任何挂载环境（OpenClaw、Codex、Claude Code、Cursor 等）里长期陪伴科研用户时，通过日常对话逐步更新 USER.md、SOUL.md、MEMORY.md 与 memory/YYYY-MM-DD.md，并周期性做一次像“做梦”一样的深度整理，使记忆文件逐步向高质量科研数字分身靠拢。
...
这里的专业结构参考“科研数字分身”的科研画像框架；技术机制采用“前台增量更新 + 后台周期性巩固”的方式。
...
上面的 `认知风格`、`学术动机`、`人格特征` 是长期关注维度，也是“科研数字分身”里需要持续沉淀的核心部分，不等同于必须执行正式量化工具。
